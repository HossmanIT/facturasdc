...
def exportar_registros():
    try:
        ...
        try:
            ...
            firebird_cursor.execute("SELECT COUNT(*) FROM FACTF01")
        ...
        try:
            sql_cursor.execute("""
            IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'SQLFACTF01')
            CREATE TABLE SQLFACTF01 (
        ...
        try:
            sql_cursor.execute("SELECT CVE_DOC FROM SQLFACTF01")
        ...
        try:
            ...
            FROM FACTF01 f JOIN CLIE01 c ON f.CVE_CLPV = c.CLAVE JOIN MONED01 m ON f.NUM_MONED = m.NUM_MONED JOIN VEND01 v ON f.CVE_VEND = v.CVE_VEND
        ...
        if registros:
            try:
                ...
                sql_cursor.executemany(
                    "INSERT INTO SQLFACTF01 (CVE_DOC, NOMBRE, CVE_PEDI, FECHA_DOC, FECHA_VEN, MONEDA, TIPCAMB, IMPORTE, IMPORTEME, VENDEDOR, SINCRONIZADO) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                    registros
                )
                sql_conn.commit()
